            ...
            if (FD_ISSET(sockfd, &readfds)) {
                ...
                if((relay_data->type == 0x61) && (relay_data->circuit_id == in_c_id)){
                    if((flag_protocol == 1) && (out_port == 65535)){
                        ...
                        int temp = numbytes_start -4;
                        fprintf(stderr,"NUMBYTES HERERE = %d\n", temp);
                        char buffer_tosend[MAX_TCP_BUF_LEN];
                        memset(&buffer_tosend,'\0',sizeof buffer_tosend);
                        for(int i=0;i<(numbytes_start-4);i++){  
                            buffer_tosend[i] = buffer[4+i]; 
                        }
                        struct msghdr buf;
                        struct iovec iov[1];
                        iov[0].iov_base= icmp;
                        iov[0].iov_len= temp - sizeof(struct iphdr);
                        ...
                        fprintf(stderr,"Sending TCP TO INTERNET");
                    }
