/*
This file contains the code stage 7 This acts as proxy and also defines tunneling.
functions:
ip_address_info_stage7 : used for getting ip address from ethernet interfaces
server_connection_stage7: for creating proxy connection
ip_checksum_stage7- for computing the checksum (// copied the entire code for calculating checksum from http://www.microhowto.info/howto/calculate_an_internet_protocol_checksum_in_c.html#idp22752)
tunnel_reader_stage7 function handles the proxy packets
*/
...
int server_connection_stage7(){ 
    ...
    proxy_addr.sin_addr.s_addr = inet_addr(ip_address_info_stage7("eth0"));
...
uint16_t ip_checksum_stage7(void* vdata,size_t length) {
...
int tun_alloc_stage7(char *dev, int flags) 
{
    struct ifreq ifr;
    int fd, err;
...
int tunnel_reader_stage7()
{
    ...
    int tun_fd = tun_alloc_stage7(tun_name, IFF_TUN | IFF_NO_PI);
    ...
    while(1) 
    {
        ...
        // one or both of the descriptors have data
            if (FD_ISSET(sockfd_proxy, &readfds)) {
                ...
                ip->daddr = inet_addr(ip_address_info_stage7("eth0"));
                ip->check = 0;           
                ip->check = ip_checksum_stage7(ip,20);
...
int main_stage7() 
{
    address_list_global[0]=inet_addr(ip_address_info_stage7("eth1"));
    address_list_global[1]=inet_addr(ip_address_info_stage7("eth2"));
    address_list_global[2]=inet_addr(ip_address_info_stage7("eth3"));
    address_list_global[3]=inet_addr(ip_address_info_stage7("eth4"));
    address_list_global[4]=inet_addr(ip_address_info_stage7("eth5"));
    address_list_global[5]=inet_addr(ip_address_info_stage7("eth6"));

    sockfd_proxy = server_connection_stage7();
    ...
    for(int index_router =1; index_router<=number_routers;index_router++){
        if (fork() == 0){
            client_connection_stage7(index_router);
        }
        else{
            char buf[100];
            int numbytes;
    ...
    global_key = circuit_creation_stage7(number_routers,manitor_hops); 
    tunnel_reader_stage7();  
    close(sockfd_proxy);
    return 0;
}
